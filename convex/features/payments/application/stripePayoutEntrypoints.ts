...
export const releaseHostPayoutInternal = internalAction({
  ...
  async handler(ctx, args): Promise<{ released: boolean; reason?: string; transferId?: string }> {
    const gate = await ctx.runMutation(unsafeInternal.stripePayouts.markBookingCompletedAndEligibleInternal, {
    ...
    const payment = await ctx.runQuery(unsafeInternal.stripePayouts.getPaymentForPayoutInternal, {
      paymentId: args.paymentId,
    });
    if (!payment) {
    ...
    if (!payment.hostStripeConnectAccountId) {
      await ctx.runMutation(unsafeInternal.stripe.updatePaymentPayoutErrorInternal, {
        paymentId: payment._id,
      });
      return { released: false, reason: "missing_host_connect_account" };
    }
    if (!payment.hostStripePayoutsEnabled) {
      await ctx.runMutation(unsafeInternal.stripe.updatePaymentPayoutBlockedInternal, {
        paymentId: payment._id,
      });
    ...
    try {
      ...
      await ctx.runMutation(unsafeInternal.stripe.markPaymentTransferredInternal, {
        paymentId: payment._id,
...
export const retryHostPayoutsForHostInternal = internalAction({
  args: {
    hostId: v.id("hosts"),
  ...
  async handler(ctx, args) {
    ...
    const [errorPayments, blockedPayments, eligiblePayments] = await Promise.all([
      ctx.runQuery(unsafeInternal.stripePayouts.listPaymentsByHostPayoutStatusInternal, {
        hostId: args.hostId,
        payoutStatus: "error",
      }),
      ctx.runQuery(unsafeInternal.stripePayouts.listPaymentsByHostPayoutStatusInternal, {
    ...
    const candidates = [...errorPayments, ...blockedPayments, ...eligiblePayments]
      .filter((payment) => {
        if (payment.status !== "paid") return false;
        if ((payment.paymentStrategy ?? "platform_transfer_fallback") !== "platform_transfer_fallback") {
          return false;
        }
        return typeof payment.releaseAt === "number" && payment.releaseAt <= Date.now();
      })
    ...
    for (const payment of candidates) {
      attempted += 1;
      const result = await ctx.runAction(unsafeInternal.stripePayouts.releaseHostPayoutInternal, {
        paymentId: payment._id,
      });
      if (result?.released) {
        released += 1;
...
export const listPaymentsByHostPayoutStatusInternal = internalQuery({
  args: {
    hostId: v.id("hosts"),
    payoutStatus: v.union(v.literal("error"), v.literal("blocked"), v.literal("eligible")),
  },
  async handler(ctx, args) {
    return await ctx.db
      .query("payments")
      .withIndex("by_host_payout_status", (q) => q.eq("hostId", args.hostId).eq("payoutStatus", args.payoutStatus))
      .collect();
  },
});
