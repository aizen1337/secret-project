...
function normalizeAddressLanguage(rawLanguage: string | undefined) {
  const fallback = "en";
...
export const consumeExternalApiBudgetInternal = internalMutation({
  args: {
    action: v.union(
  ...
  async handler(ctx, args) {
    ...
    const now = Date.now();
    const bucketStart = now - (now % Math.max(1000, Math.round(args.windowMs)));

    const existing = await ctx.db
      .query("external_api_usage")
      .withIndex("by_user_action_bucket", (q) =>
        q.eq("userId", user._id).eq("action", args.action).eq("bucketStart", bucketStart),
      )
      .first();

    if (existing) {
      if (existing.count >= args.limit) {
        throw new Error("RATE_LIMITED: Too many external requests. Please try again shortly.");
      }
      await ctx.db.patch(existing._id, {
        count: existing.count + 1,
        updatedAt: now,
      });
      return { remaining: Math.max(0, args.limit - (existing.count + 1)) };
    }

    await ctx.db.insert("external_api_usage", {
      userId: user._id,
      action: args.action,
      bucketStart,
      count: 1,
...
export const verifyAndAutofillCarFromVinUncached = internalAction({
  ...
  async handler(ctx, args): Promise<VinLookupResult> {
    await ctx.runMutation(internal.cars.consumeExternalApiBudgetInternal, {
      action: "vin_lookup",
      limit: VIN_LOOKUP_LIMIT_PER_WINDOW,
      windowMs: EXTERNAL_API_RATE_WINDOW_MS,
    });
...
export const searchAddressesUncached = internalAction({
  ...
  async handler(ctx, args): Promise<AddressSuggestion[]> {
    await ctx.runMutation(internal.cars.consumeExternalApiBudgetInternal, {
      ...
      limit: ADDRESS_SEARCH_LIMIT_PER_WINDOW,
      windowMs: EXTERNAL_API_RATE_WINDOW_MS,
    });
...
export const resolveAddressDetailsUncached = internalAction({
  ...
  async handler(ctx, args): Promise<AddressDetails> {
    await ctx.runMutation(internal.cars.consumeExternalApiBudgetInternal, {
      ...
      limit: ADDRESS_DETAILS_LIMIT_PER_WINDOW,
      windowMs: EXTERNAL_API_RATE_WINDOW_MS,
    });
...
export const resolveAddressDetails = action({
  ...
  async handler(ctx, args): Promise<AddressDetails> {
    ...
    const language = normalizeAddressLanguage(args.language);
    return await addressDetailsCache.fetch(ctx, { placeId, language });
  },
});
...
async function enrichCarsForPromotionSignals(ctx: any, cars: any[]) {
  ...
  const hostUserIds = Array.from(new Set(Array.from(hostUserByHostId.values())));

  const [hostUsers, reviewBuckets, bookingBuckets] = await Promise.all([
    Promise.all(hostUserIds.map((userId) => ctx.db.get(userId as any))),
    Promise.all(
      carIds.map((carId) =>
        ctx.db
          .query("reviews")
  ...
  const hostReviewStatsByUserId = new Map<string, { avg: number; count: number }>();
  hostUsers.forEach((hostUser, index) => {
    const userId = hostUserIds[index];
    const avg = Number(hostUser?.avgRating ?? 0);
    const count = Number(hostUser?.reviewCount ?? 0);
    hostReviewStatsByUserId.set(userId, {
      avg: Number.isFinite(avg) ? avg : 0,
      count: Number.isFinite(count) ? count : 0,
    });
  });

  const carReviewStatsByCarId = new Map<string, { sum: number; count: number }>();
  for (let index = 0; index < carIds.length; index += 1) {
    const carId = carIds[index];
    const reviews = reviewBuckets[index] ?? [];
    let sum = 0;
    let count = 0;
  ...
  for (let index = 0; index < carIds.length; index += 1) {
    ...
    for (const booking of bookings) {
      ...
      aggregate.bookingCount180d += 1;
      aggregate.totalRentalDays180d += overlapDays;
    }

    bookingStatsByCarId.set(carId, aggregate);
  }
  ...
  return cars.map((car) => {
    ...
    const hostReviewStats = hostUserId
      ? hostReviewStatsByUserId.get(hostUserId) ?? { avg: 0, count: 0 }
      : { avg: 0, count: 0 };
  ...
  async handler(ctx, args) {
    ...
    const chunk = await ctx.db
      ...
      .paginate({
        ...
        // Convex allows only one paginated query per function. Overfetch once,
        // then filter down to the requested page size.
    ...
    return {
      page: filteredPage,
      isDone: chunk.isDone,
      continueCursor: chunk.continueCursor,
    };
  },
});
