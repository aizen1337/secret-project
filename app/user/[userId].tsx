...
type UserProfileTab = "renter" | "host";
...
export default function UserProfileInspectionScreen() {
  ...
  const profile = useQuery(
    (api as any).users.getPublicUserProfile,
    isSignedIn && userIdParam
      ? {
  ...
  const tabData = activeTab === "renter" ? profile.renter : profile.host;
  const completedCount =
    activeTab === "renter" ? profile.renter.completedBookingsCount : profile.host.completedRentalsCount;
  const completedLabel =
    activeTab === "renter" ? t("userProfile.completedBookings") : t("userProfile.completedRentals");
      ...
      <ScrollView className="flex-1 px-4 pt-4" showsVerticalScrollIndicator={false}>
        ...
        <View className="mt-4 flex-row rounded-xl bg-secondary p-1">
          <Pressable
            ...
            className={`flex-1 rounded-lg py-2.5 ${activeTab === "renter" ? "bg-card" : ""}`}
          >
            <Text
          ...
          <Pressable
            ...
            className={`flex-1 rounded-lg py-2.5 ${activeTab === "host" ? "bg-card" : ""}`}
          >
            <Text
              className={`text-center font-medium ${
        ...
        {activeTab === "host" && !profile.host.isHost ? (
          <View className="mt-4 rounded-2xl border border-border bg-card p-4">
            ...
            <View className="gap-3">
              {tabData.reviews.map((review) => (
                <ReviewCard
                  key={review.id}
                  review={review}
                  fallbackAuthor={t("common.unknown")}
                  starColor={starColor}
                />
              ))}
            </View>
