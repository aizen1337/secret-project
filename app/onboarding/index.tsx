...
export default function OnboardingScreen() {
  ...
  const hostPayoutStatus = useQuery(api.users.getHostPayoutStatus, isSignedIn ? {} : "skip");

  const beginSignupOnboarding = useMutation(api.users.beginSignupOnboarding as any);
  const setOnboardingRole = useMutation(api.users.setOnboardingRole as any);
  const finalizeOnboarding = useMutation(api.users.finalizeOnboarding as any);
  const startRenterIdentityCheck = useAction(api.verification.startRenterIdentityCheck as any);
  const startRenterDriverLicenseCheck = useAction(api.verification.startRenterDriverLicenseCheck as any);
  const syncRenterVerificationSession = useAction(api.verification.syncRenterVerificationSession as any);
  const createHostOnboardingLink = useAction(api.stripeConnect.createHostOnboardingLink as any);
  const refreshHostConnectStatus = useAction(api.stripeConnect.refreshHostConnectStatus as any);
