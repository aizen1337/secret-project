...
export default function BookingDetailsScreen() {
  ...
  const createBookingReview = useMutation(bookingsApi.createBookingReview);
  const createReservationPayNowSession = useAction(bookingsApi.createReservationPayNowSession);
  const retryHostPayoutTransfer = useAction(bookingsApi.retryHostPayoutTransfer);
  ...
  const handleRetryPayout = async () => {
    ...
    try {
      const result = await retryHostPayoutTransfer({
        bookingId: details.booking.id as any,
      });
      if (result?.released) {
        toast.success(t("bookingDetails.payoutRetrySuccess"));
      } else {
        const readableReason = toReadableFallback(String(result?.reason ?? "unknown"));
        toast.error(t("bookingDetails.payoutRetryFailed", { reason: readableReason }));
      }
    } catch (error) {
      toast.error(toLocalizedErrorMessage(error, t));
    } finally {
    ...
    <View>
      <View className="pb-6 border-b border-border/50">
        ...
        {canRetryPayout ? (
          <Pressable
            onPress={() => {
              void handleRetryPayout();
            }}
            disabled={pendingPayoutRetry}
            className={`mt-3 rounded-lg py-2 items-center ${
              pendingPayoutRetry ? "bg-primary/60" : "bg-primary"
            }`}
          >
            <Text className="text-sm font-semibold text-primary-foreground">
              {pendingPayoutRetry ? t("bookingDetails.retryingPayout") : t("bookingDetails.retryPayout")}
            </Text>
          </Pressable>
        ) : null}
        ...
        {details.booking?.status === "completed" ? (
          <View className="mt-4 rounded-lg border border-border p-3">
            <Text className="text-sm font-semibold text-foreground mb-2">
              {isHostViewer ? t("profile.reviews.rateRenter") : t("profile.reviews.rateHost")}
            </Text>
            {details.myReview ? (
              <View>
              ...
              <View>
                <View className="flex-row gap-1 mb-2">
                  {[1, 2, 3, 4, 5].map((rating) => (
                    <Pressable
                      ...
                      onPress={() => setReviewDraft((prev) => ({ ...prev, rating }))}
                    >
                      <Ionicons
                        name={rating <= reviewDraft.rating ? "star" : "star-outline"}
                        size={18}
                        color={getTokenColor(mode, "ratingStar")}
                      />
                    </Pressable>
                ...
                <TextInput
                  ...
                  onChangeText={(comment) => setReviewDraft((prev) => ({ ...prev, comment }))}
                ...
                <Pressable
                  ...
                  className={`mt-2 rounded-lg py-2 items-center ${
                    pendingReview ? "bg-primary/60" : "bg-primary"
                  }`}
                >
                  <Text className="text-sm font-semibold text-primary-foreground">
